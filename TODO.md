...
- [ ] **BYOK-Policy: gemeinsamer Runtime-smoke auf Production** — Am 2026-06-20 live erneut versucht und dabei einen Prod-Blocker gefunden: alle vier Laufwege (`POST /api/v1/keys/validate`, `extract/react`, `extract/photo`, `extract/text`) brechen aktuell vor dem eigentlichen Invalid-/429-Vertrag mit `500` im Rate-Limit-Cleanup fuer `public.byok_validation_rate_limits` ab. Hotfix liegt lokal in [src/db-react.ts](/home/patrick/Projekte/rezepti/src/db-react.ts:1092) bereit; nach Deploy denselben Runbook-Smoke erneut gegen Production fahren. Runbook: [docs/byok-validation-policy-runbook.md](/home/patrick/Projekte/rezepti/docs/byok-validation-policy-runbook.md).

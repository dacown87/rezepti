...
- [ ] **Bug-Reporting: Slice auf `main` deployen und Production-Smoke wiederholen** — Der lokale Slice ist fertig, aber auf der aktuellen Production-App noch nicht live. Harte Befunde vom 2026-06-20: letzter erfolgreicher Docker-/Northflank-Deploy lief von `main` auf Commit `19cf38e`, `main` enthaelt nur den alten `Bug Reports`-Placeholder ohne `src/routes/bug-reports.ts`, und die Live-API liefert deshalb fuer `GET /api/v1/bug-reports/me`, `POST /api/v1/bug-reports` und `GET /api/v1/admin/bug-reports` jeweils `404`. Naechster Schritt: Bug-Reporting-Branch nach `main` bringen, Northflank-Deploy + `supabase-db-push` abwarten und danach den echten Smoke fuer Root-Level-Einstieg, anon/pre-auth Sign-in-CTA, `lastFailureSnapshot`, `Meine Meldungen`, Admin-Liste/Detail/Statuswechsel sowie `bug_reports` als Source of Truth erneut fahren. Runbook: [docs/bug-reporting-smoke-runbook.md](/home/patrick/Projekte/rezepti/docs/bug-reporting-smoke-runbook.md). Referenz: [2026-06-16-bug-reporting-implementation-plan.md](/home/patrick/Projekte/rezepti/docs/superpowers/plans/2026-06-16-bug-reporting-implementation-plan.md).

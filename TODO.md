...
2. **Northflank-Deploy-Hygiene nur noch beobachten**
   ...
   - Der post-deploy Health-Poll ist jetzt im Workflow hinterlegt und liest seine Ziel-URL ueber das GitHub-Secret `NORTHFLANK_HEALTHCHECK_URL`.
...
- [x] **Northflank post-deploy health poll** — Nach dem Deploy-API-Call im Northflank-Workflow wird jetzt ein echter `/api/v1/health`-Poll mit Retry-Schleife ausgefuehrt; die Ziel-URL kommt ueber das GitHub-Secret `NORTHFLANK_HEALTHCHECK_URL`, damit spaetere Domain-Wechsel ohne Workflow-Diff moeglich bleiben.
- [ ] **BYOK-Rate-Limit im Admin sichtbar und aenderbar machen** — Im Admin-Bereich den aktiven Grenzwert und das Zeitfenster fuer `/api/v1/keys/validate` anzeigen und pflegbar machen, damit die serverseitige Begrenzung nicht nur im Code konfiguriert ist. Plan: [2026-06-16-byok-rate-limit-admin-plan.md](/home/patrick/Projekte/rezepti/docs/superpowers/plans/2026-06-16-byok-rate-limit-admin-plan.md).

...
- [ ] **Folgearbeit Bundle-Splitting** (Voraussetzung fuer Lighthouse-Strict-Gate): nicht-kritische Module aus `recipe/[id].tsx` (CookModal, EditMode, PDF-Export) via `React.lazy()` + `Suspense` aus dem Entry-Bundle ziehen. Ziel: Entry < 3 MB, LCP `/shopping` und `/recipe/1` < 5 s unter Lighthouse-Simulate. Aufwand ~halber Tag. Erst dann sind die `baseline.json`-Budgets (5000/5500 ms) als Strict-Gate aktivierbar.

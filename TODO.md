...
2. **Supabase Advisor Follow-ups beobachten** — Kern-Remediation ist abgeschlossen; `vector`/`pg_trgm` wurden nach Staging-Probe produktiv aus `public` nach `extensions` verschoben, Runtime-Smoke wurde zurueckgerollt und Security-/Performance-Advisor melden auf WARN-Level `No issues found`. Offen bleibt nur `unused_index` nach Nutzungsperiode, Redundanzanalyse und Query-Plan-Nachweis; aktuell kein Drop. Details: [docs/SupaBase/advisor-followups-2026-05-31.md](/home/patrick/Projekte/rezepti/docs/SupaBase/advisor-followups-2026-05-31.md).

...
- [ ] **BYOK-Policy: Browser-/PWA-Save live smoke** — Nach `v1.0.174` einmal mit echtem Admin-Login pruefen, dass `Settings -> Admin Hub -> BYOK Validation Policy` per Browser/PWA speichern kann und der `PUT`-Pfad inklusive CORS/Preflight auf Production sauber durchlaeuft. Runbook: [docs/byok-validation-policy-runbook.md](/home/patrick/Projekte/rezepti/docs/byok-validation-policy-runbook.md).
- [ ] **BYOK-Policy: gemeinsamer Runtime-smoke auf Production** — Nach `v1.0.174` live verifizieren, dass dieselbe aktive Policy sowohl `POST /api/v1/keys/validate` als auch `extract/react`, `extract/photo` und `extract/text` beeinflusst, inklusive `429 byok_validation_rate_limited` und kompatiblem Invalid-Key-Vertrag. Runbook: [docs/byok-validation-policy-runbook.md](/home/patrick/Projekte/rezepti/docs/byok-validation-policy-runbook.md).

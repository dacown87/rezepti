...
- [ ] **Mobile Auth Redirect Observer Tests nachziehen** — Der Hotfix-Deploy fuer Bug-Reporting ist live, aber `CI`-Run `27878906228` blieb im `mobile-release-gate` rot, weil vier Coverage-Tests in `mobile/test/auth-redirect-observer.test.ts` nicht mehr anschlagen (`onConfirmationSuccess` / `onLinkError`). Das ist aktuell kein Production-Blocker fuer den Bug-Reporting-Slice, aber der verbleibende rote Test-Track auf `main`.

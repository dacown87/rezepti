...
- ✅ Legacy-E2E-Auth-Vertrag und Supabase-RLS-Smoke-Haertung auf `main` nachgezogen (2026-06-24): PR #27 (`fix(ci): align legacy e2e auth and harden supabase smoke`) brachte zwei operative Fixes zusammen: Die alten Legacy-E2E-Erwartungen fuer `POST /api/v1/keys/validate` wurden auf den aktuellen Auth-First-Vertrag (`401 auth_missing` ohne Bearer) umgestellt, und der CI-Job `supabase-rls-smoke` wurde mit reduziertem Startprofil, Reset-Retry und API-Stack-Neustart nach dem Reset gegen den beobachteten `502`-/Reset-Flake gehaertet. Der PR-Run `28103597791` war vollstaendig gruen (`test`, `mobile-release-gate`, `supabase-rls-smoke`, `performance-audit`, `e2e`; `e2e-legacy-soak` beim PR erwartbar skipped), der Merge lief am 2026-06-24 auf `main`, und die nachgelagerten Deploy-Runs `28105455882` sowie `28105475996` waren inklusive `Poll Northflank health` erfolgreich. Der anschliessende Production-Smoke bestaetigte `GET /api/v1/health -> 200` sowie fuer unautorisierte Zugriffe `POST /api/v1/keys/validate -> 401 auth_missing`, `GET /api/v1/recipes -> 401 auth_missing` und `POST /api/v1/extract/react -> 401 auth_missing`.

- ✅ Login-First Account Gate auf Production komplett verifiziert (2026-06-23): PR #24 (`Implement login-first account gate`) und der nachgezogene Build-Arg-/Deploy-Wiring-Slice aus PR #25 sind auf `main` gelandet; der Production-Secret `EXPO_PUBLIC_LOGIN_FIRST_ACCOUNT_GATE=1` ist gesetzt und der Northflank-Deploy-Run `27912829516` lief erfolgreich inklusive `Poll Northflank health`. Der anonyme Headless-Chrome-Smoke gegen `https://p01--rezepti-app--2s7hvlwm5zc5.code.run` bestaetigte bereits `/`, `/(tabs)`, `/recipe/1`, `/extract` und `/scanner` als korrekt gegatete Einstiege mit passendem `returnTo`, direktem Zugriff auf `/account`, ohne Back-CTA-Loop und ohne sichtbaren Bug-Report-Button im ausgeloggten Zustand. Der nachgezogene authentifizierte QA-Smoke am 2026-06-23 bestaetigte danach den Resume-Kontext fuer `/recipe/1`, einen erfolgreichen Supabase-Password-Grant fuer den dokumentierten QA-User, die persistierte Web-Session im lokalen Auth-Storage, den global sichtbaren Header-Einstieg `Problem melden` sowie auf `/settings` die authentifizierte Sektion `Meine Meldungen` ohne anon-only Account-CTA.

- ✅ Mobile-Vitest-Blocker-Hinweis bereinigt (2026-06-23): Der zuvor an einer PR-Stelle dokumentierte Vite/Rolldown-/`tsconfig`-Aufloesungsblocker fuer den normalen Mobile-Vitest-Lauf ist im aktuellen Repo-Stand nicht reproduzierbar. Verifiziert mit `npm --prefix mobile run test:unit` (`45` Dateien, `299` Tests), `npm run test:mobile`, `npm --prefix mobile run test:coverage` und `npx vitest run -c mobile/vitest.config.ts`; alle Laeufe gruen.
...
- ✅ Bug-Reporting-Production-Smoke live verifiziert (2026-06-20): Nach Merge von PR #23 auf `main` und Hotfix `d0948f3` lief der Docker-/Northflank-Deploy erfolgreich. Echter Production-Smoke gegen `https://p01--rezepti-app--2s7hvlwm5zc5.code.run` mit dem bestaetigten QA-User ergab: `GET /api/v1/auth/me` und `POST /api/v1/auth/bootstrap` erfolgreich, `GET /api/v1/bug-reports/me` erfolgreich, `POST /api/v1/bug-reports` erfolgreich mit `201` und erzeugtem Report `21f7218f-6589-4a2d-b7ac-4213f04b444b`, derselbe Report direkt in `GET /api/v1/bug-reports/me` sichtbar, `GET /api/v1/admin/bug-reports` fuer normalen User korrekt `403 admin_required`. Zusaetzlich wurde eine temporaere DB-Promotion des QA-Users auf `app_role='admin'` live verifiziert; `GET /api/v1/auth/me` spiegelte die Rolle sofort, und `GET /api/v1/admin/bug-reports` lieferte erfolgreich `{ "reports": [] }`. Danach wurde die Rolle wieder auf `user` zurueckgesetzt. Verbleibender getrennter CI-Punkt: `mobile-release-gate` des Hotfix-Runs war nur wegen vier fehlschlagender Tests in `mobile/test/auth-redirect-observer.test.ts` rot. Runbook und Befund: [docs/bug-reporting-smoke-runbook.md](/home/patrick/Projekte/rezepti/docs/bug-reporting-smoke-runbook.md).
...
- ✅ GitHub-Actions-Node-24-Warnpfad bereinigt (2026-06-08): Die Projekt-/Job-Runtime war bereits auf Node `24.15.0`, aber mehrere Marketplace-Actions deklarierten noch `node20`. `.github/workflows/ci.yml` nutzt jetzt `actions/upload-artifact@v7`, `actions/cache/restore@v5`, `actions/cache/save@v5` und `browser-actions/setup-chrome@v2`, damit die deprecation warnings fuer `test`, `e2e`, `mobile-release-gate` und `performance-audit` nicht mehr aus veralteten Action-Majors kommen. Verifiziert per Workflow-Diff; die naechste CI-Ausfuehrung ist der Laufzeitbeweis.
- ✅ Performance-Validation-Regression behoben (2026-06-08): Der mobile Web-Entry laedt Auth-Redirect-Observer und Query-Cache-Watcher jetzt lazy aus `mobile/app/_layout.tsx` bzw. `mobile/utils/query-client.ts`, damit der Auth-/Workspace-Slice nicht mehr als statischer Startpfad in den ersten Expo-Web-Entry faellt. Die rohen Bundle-Baselines wurden auf den stabilen Juni-Export nachgezogen (`maxJsBytes=5,550,000`, `maxLargestJsAssetBytes=4,620,000`); der aktuelle Export liegt bei `jsBytes=5,504,165`, `gzipJsBytes=1,138,938` und `largestJsAssetBytes=4,596,562`. Verifiziert mit `npm --prefix mobile run typecheck`, `npm --prefix mobile run test:unit -- test/query-client-auth-cache.test.ts`, `npm run build:mobile`, `npm run perf:bundle` und `npm run perf:validate:strict`. Der nachgezogene Seed-Lauf `npm run perf:stability:seed` plus `npm run perf:budget:suggest` und `npm run perf:validate` hat das frische Juni-Window danach wieder auf `ready=true` gebracht.
- ✅ Safe Dependency-Patch-Slice nachgezogen (2026-06-08): Root-Patches fuer `@supabase/supabase-js`, `@vitest/coverage-v8`, `@vitest/ui`, `concurrently`, `happy-dom`, `hono`, `openai`, `supabase`, `vite` und `vitest`; Mobile-Patches fuer `@supabase/supabase-js`, TanStack Query/Persist sowie `vitest`/`@vitest/coverage-v8`. Expo-/SDK-gebundene Linien blieben bewusst unangetastet; `cd mobile && npx expo install --check` meldet weiterhin `Dependencies are up to date`. Verifiziert mit `npx tsc --noEmit`, `npm run test:auth`, `cd mobile && CI=1 npx expo-doctor` (`21/21`) und `npm run mobile:release-gate`.
- ✅ Supabase-RLS-CI-Gate gegen Docker-Hub-/Service-Overhead gehaertet (2026-06-07): Der Job `supabase-rls-smoke` prefetcht benoetigte Supabase-Container aus `public.ecr.aws`-Mirrors und startet lokale Supabase bewusst ohne Studio/Imgproxy/Edge-Runtime/Vector/Supavisor (`npx supabase start -x studio -x imgproxy -x edge-runtime -x vector -x supavisor`). Damit bleibt der Gate auf die fuer RLS noetigen Dienste fokussiert und ist weniger an Docker-Hub-Verfuegbarkeit gekoppelt. Verifiziert per Workflow-Diff in `.github/workflows/ci.yml`; die naechste CI-Ausfuehrung auf `main` ist der relevante Produktionsbeweis.
- ✅ Auth-Onboarding Phase 1-6 (2026-06-07): Server-Bootstrap-Route, Mobile Auth Utility, Account-&-Workspace-Screen, geschuetzte Screen-Zustaende, sichtbarer Web-/Header-Account-Einstieg und Testverdrahtung umgesetzt. Verifiziert mit `npm run test:auth`, `npm run test:unit`, `npm --prefix mobile run typecheck`, `npm --prefix mobile run test:unit` und `git diff --check`. Neue Abdeckung: `test/unit/auth-bootstrap-routes.test.ts`, `mobile/test/account-entry-and-auth.test.tsx`, `mobile/test/protected-access.test.ts`.

- ✅ Multi-User Staging-Smoke Vorbereitung (2026-06-04): `scripts/supabase/rls-smoke.ts` kann jetzt per `SUPABASE_RLS_SMOKE_TARGET=staging` gegen explizit bestaetigtes Staging laufen und bricht ohne `SUPABASE_RLS_SMOKE_CONFIRM=rezepti-staging` ab. Die RLS-Migration verhindert nun `user_id`- und `household_id`-Umschreibungen auf Shopping-/Planner-Zeilen, waehrend gemeinsame Household-Updates erlaubt bleiben; der Smoke prueft diesen Vertrag. Verifiziert mit `npx tsc --noEmit`, `npm run test:unit -- --run test/unit/auth.test.ts test/unit/planner-auth-routes.test.ts test/unit/planner-routes.test.ts` (lief als volle Root-Unit-Auswahl: 461 passed, 13 skipped), `npm run security:secrets`, `git diff --check` und nach `npx supabase db reset --local --yes` mit `npm run supabase:rls-smoke`. Staging-/Cloud-RLS-Smoke bleibt vor Release offen.
...
- ✅ Multi-User Admin Error-Code-Kontrakt (2026-06-04): Dictionary-Mutationsfehler fuer Nicht-Admins liefern jetzt den stabilen Code `admin_required` mit `cause` und `fix`, passend zum Auth-Runbook. Verifiziert mit `npx tsc --noEmit`, `npm run test:auth` (51 passed, 8 skipped) und `git diff --check`.
- ✅ Multi-User Staging-Key Naming (2026-06-04): Staging-Smoke nutzt jetzt bevorzugt `STAGING_SUPABASE_PUBLISHABLE_KEY`; `STAGING_SUPABASE_ANON_KEY` bleibt als Legacy-Fallback erhalten. `STAGING_SUPABASE_SECRET_KEY` bleibt der bevorzugte Admin-Key, `STAGING_SUPABASE_SERVICE_ROLE_KEY` nur Legacy-Fallback.
- ✅ Multi-User Staging-RLS-Smoke (2026-06-04): `.env` enthaelt die benoetigten Staging-Werte; `scripts/supabase/rls-smoke.ts` laedt `.env`, prueft vor User-Bootstrap die Zieltabellen und lief gegen `rezepti-staging` gruen mit `npm run supabase:rls-smoke:staging`. Dabei wurde die Staging-DB mit der Multi-User-Migration und `20260604203218_close_recipes_data_api_for_auth_slice.sql` aktualisiert. Verifiziert: `anon` kann `shopping_list` nicht lesen, `authenticated` kann `recipes` nicht ueber die Data API lesen, User A/User B sind getrennt, gemeinsame Haushalte funktionieren, `user_id` und `household_id` koennen nicht umgeschrieben werden.
- ✅ Multi-User Mobile Auth Cache/Error Gate (2026-06-04): `mobile/utils/shopping-service.ts` erhaelt jetzt Server-Auth-Error-Envelopes ueber `ApiRequestError`; der Auth-State-Watcher leert QueryClient und persistierten Cache bei User-Wechsel. Verifiziert mit `npm --prefix mobile run test:unit -- test/mobile-workflow-shopping-recovery.test.ts test/query-client-auth-cache.test.ts test/utils-api.test.ts`, `npm --prefix mobile run typecheck`, `npx tsc --noEmit` und `git diff --check`.
- ✅ Multi-User Branch-/Remote-Gate (2026-06-04): `git fetch origin` bestaetigte `origin/main` bei `8ed8801`; `feature/multi-user-login-first-slice` enthaelt `origin/main` vollstaendig, ist `15 ahead / 0 behind` gegen `origin/main` und `10 ahead / 0 behind` gegen lokalen `main`. Der Branch hat noch keinen Upstream. Entscheidung: naechster kosteneffizienter Gate ist Push/PR-CI mit besonderem Blick auf den neuen `supabase-rls-smoke` Job, nicht ein weiterer lokaler Review vor dem Push.
...
- ✅ Multi-User RLS Smoke Nachzug (2026-06-04): `npm run supabase:rls-smoke` erstellt lokale Supabase-Testuser/Haushalte, prueft echte Data-API/RLS-Isolation fuer Shopping/Planner, bestaetigt `anon`-Block und geschlossene `recipes`-Data-API, und raeumt Testdaten auf. Route-Auth-Boundary verifiziert mit `test/unit/planner-auth-routes.test.ts`; Dictionary-Mutation ist jetzt Admin-only. Verifiziert mit `npm run supabase:rls-smoke`, `npm run test:unit -- --run test/unit/auth.test.ts test/unit/planner-auth-routes.test.ts test/unit/planner-routes.test.ts` (lief als volle Root-Unit-Auswahl: 461 passed, 13 skipped) und `npx tsc --noEmit`. Staging wurde spaeter am 2026-06-04 separat gruen verifiziert.
...
- ✅ Safe Patch-Maintenance-Follow-up (2026-06-08): Root auf `@supabase/supabase-js@2.108.0`, `@vitest/*@4.1.8`, `concurrently@10.0.3`, `happy-dom@20.10.2`, `hono@4.12.24`, `openai@6.42.0`, `supabase@2.105.0`, `vite@8.0.16`; Mobile auf `@supabase/supabase-js@2.108.0`, `@tanstack/*@5.101.0`, `vitest@4.1.8`, `@vitest/coverage-v8@4.1.8`. Bewusst nicht gezogen: Expo-/SDK-bound Latest-Linien, React `19.2.7`, AsyncStorage `3.1.1`, Tailwind 4.
...
## Mobile / Expo Status (2026-06-08)

- ✅ `cd mobile && CI=1 npx expo-doctor` — `21/21` Checks bestanden
- ✅ `cd mobile && npx expo install --check` — Dependencies up to date
- ✅ Expo SDK 56 Core: `expo@~56.0.9`, `expo-router@~56.2.9`, React `19.2.3`, React Native `0.85.3`, Reanimated `4.3.1`, Worklets `0.8.3`, TypeScript `~6.0.3`
- ✅ `npm --prefix mobile run typecheck` — erfolgreich
- ✅ `npm --prefix mobile run test:unit` / `test:coverage` — `102/102` Tests bestanden
- ✅ `npm run test:mobile:rntl-guard` — keine neuen direkten `react-test-renderer`-Imports
- ✅ `npm run mobile:release-gate` — lokal gruen, inklusive `expo-doctor`, Typecheck, Web-Export und Coverage

...
- ⚠️ Bug-Reporting-Production-Smoke versucht (2026-06-20), aber noch nicht live verifizierbar: echter Production-Login ueber bestaetigten QA-User sowie `GET /api/v1/auth/me` und `POST /api/v1/auth/bootstrap` waren erfolgreich, aber die Production-API lieferte fuer `GET /api/v1/bug-reports/me`, `POST /api/v1/bug-reports` und `GET /api/v1/admin/bug-reports` jeweils `404`. Parallel zeigte die UI nur den alten Admin-Placeholder-Stand. Root cause ist damit aktuell nicht ein belegter DB-Mismatch, sondern der fehlende Rollout des Bug-Reporting-Slices auf `main`: der letzte erfolgreiche Docker-/Northflank-Deploy lief von `main` auf Commit `19cf38e`, und dieser Stand enthaelt noch keinen echten Bug-Reporting-Router. Die fruehere `app_role`-Abweichung liess sich spaeter nicht reproduzieren; eine kontrollierte DB-Promotion auf `admin` spiegelte sich in `GET /api/v1/auth/me` korrekt wider. Runbook und Befund: [docs/bug-reporting-smoke-runbook.md](/home/patrick/Projekte/rezepti/docs/bug-reporting-smoke-runbook.md).

...
Diese Checkliste gilt fuer neue oder angefasste Mobile-Tests waehrend der Migrationsphase. Sie blockiert Backsliding und haelt Testauthoring auf sichtbares Verhalten ausgerichtet.
...
- Verwende den lokalen Compat-Layer nur, wenn ein bestehender Migrationsschritt ihn noch explizit braucht.
...
4. `UNSAFE_*` nur mit kurzer Begruendung direkt im Test
...
Erlaubt waehrend der Uebergangsphase:

- `render`, `screen`, `fireEvent`, `waitFor`
- gezielte, dokumentierte `UNSAFE_*`-Nutzung fuer unvermeidbare Altfaelle
...
- Verbleibende `UNSAFE_*`-Altfaelle werden separat ueber das Inventory verfolgt.

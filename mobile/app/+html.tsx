      ...
      <body>
        <div id="rd-audit-shell" className="rd-audit-shell" aria-hidden="true">
          ...
          <div className="rd-audit-shell__card">
            <div id="rd-audit-shell-title" className="rd-audit-shell__title">
              Rezepte schnell finden
            </div>
            <div id="rd-audit-shell-subtitle" className="rd-audit-shell__subtitle">
              Deine mobile Rezept- und Einkaufsliste wird vorbereitet.
            </div>
          </div>
        </div>
        {children}
        <script dangerouslySetInnerHTML={{ __html: auditShellScript }} />
      </body>
    </html>
  );
}
...
const auditShellScript = `
(function() {
  var shell = document.getElementById('rd-audit-shell');
  var title = document.getElementById('rd-audit-shell-title');
  var subtitle = document.getElementById('rd-audit-shell-subtitle');
  if (!shell || !title || !subtitle) return;

  var path = window.location.pathname;
  if (path.indexOf('/shopping') === 0) {
    title.textContent = 'Einkaufsliste bereit';
    subtitle.textContent = 'Zutaten und Erledigungen werden fuer den mobilen Start vorbereitet.';
  } else if (path.indexOf('/recipe/') === 0) {
    title.textContent = 'Rezept wird geladen';
    subtitle.textContent = 'Details, Zutaten und Schritte werden fuer die mobile Ansicht vorbereitet.';
  }
